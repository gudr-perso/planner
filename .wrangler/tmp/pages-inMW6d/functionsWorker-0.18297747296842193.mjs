...
// api/admin/users/[id]/sessions.js
async function onRequestDelete({ params, env, data }) {
  if (data.user.role !== "admin") return Response.json({ error: "Acc\xE8s refus\xE9" }, { status: 403 });
...
async function onRequestGet({ params, env, data }) {
  if (data.user.role !== "admin") return Response.json({ error: "Acc\xE8s refus\xE9" }, { status: 403 });
...
async function onRequestPut({ params, request, env, data }) {
  if (data.user.role !== "admin") return Response.json({ error: "Acc\xE8s refus\xE9" }, { status: 403 });
  const body = await request.json().catch(() => ({}));
  const user = await env.DB.prepare("SELECT * FROM users WHERE id = ?").bind(params.id).first();
  if (!user) return Response.json({ error: "Utilisateur introuvable" }, { status: 404 });
  if (params.id === data.user.id) {
    if (body.role === "user") return Response.json({ error: "Impossible de se r\xE9trograder soi-m\xEAme" }, { status: 400 });
    if (body.is_active === 0) return Response.json({ error: "Impossible de se d\xE9sactiver soi-m\xEAme" }, { status: 400 });
  }
  ...
  ).bind(name, role, is_active, (/* @__PURE__ */ new Date()).toISOString(), params.id).run();
...
async function onRequestGet2({ env, data }) {
  if (data.user.role !== "admin") {
    return Response.json({ error: "Acc\xE8s refus\xE9" }, { status: 403 });
  }
...
async function onRequestPost({ request, env, data }) {
  ...
  if (password.length < 10) {
    return Response.json({ error: "Le mot de passe doit contenir au moins 10 caract\xE8res" }, { status: 400 });
  }
  ...
  try {
    ...
    if (e.message?.includes("UNIQUE")) {
      return Response.json({ error: "Cet email est d\xE9j\xE0 utilis\xE9" }, { status: 409 });
    }
    throw e;
  }
...
async function onRequestPost2({ request, env, data }) {
  ...
  await env.DB.prepare("UPDATE users SET password_hash = ?, updated_at = ? WHERE id = ?").bind(newHash, (/* @__PURE__ */ new Date()).toISOString(), data.user.id).run();
  await env.DB.prepare("UPDATE sessions SET is_revoked = 1 WHERE user_id = ? AND id != ?").bind(data.user.id, data.sessionId).run();
  return Response.json({ ok: true });
}
__name(onRequestPost2, "onRequestPost");

// api/_lib/session.js
var COOKIE_NAME = "planner_session";
var SESSION_HOURS = 8;
function getSessionId(request) {
  const cookie = request.headers.get("Cookie") || "";
  const match2 = cookie.match(new RegExp(`(?:^|;\\s*)${COOKIE_NAME}=([^;]+)`));
...
async function createSession(db, userId) {
  ...
  const expires = new Date(now.getTime() + SESSION_HOURS * 3600 * 1e3);
  await db.prepare(
...
function sessionCookie(id, expires) {
...
async function validateSession(db, sessionId) {
  ...
  db.prepare("UPDATE sessions SET last_seen = ? WHERE id = ?").bind((/* @__PURE__ */ new Date()).toISOString(), sessionId).run().catch(() => {
  });
...
async function onRequestPost3({ request, env }) {
  ...
  if (!user.is_active) {
    return Response.json({ error: "Compte d\xE9sactiv\xE9" }, { status: 403 });
  }
  if (user.locked_until && new Date(user.locked_until) > /* @__PURE__ */ new Date()) {
    return Response.json({ error: "Compte temporairement verrouill\xE9" }, { status: 403 });
  }
...
// api/auth/me.js
async function onRequestGet3({ data }) {
...
async function onRequestPost5({ request, env }) {
  const row = await env.DB.prepare("SELECT COUNT(*) as cnt FROM users").first();
  if (row.cnt > 0) {
    return Response.json({ error: "Configuration d\xE9j\xE0 effectu\xE9e" }, { status: 403 });
  }
...
async function onRequest2(ctx) {
  ...
  if (!user) {
    return Response.json({ error: "Non authentifi\xE9" }, { status: 401 });
  }
  ctx.data.user = user;
...
var ALLOWED_FEEDS = /* @__PURE__ */ new Set([
...
var onRequest3 = /* @__PURE__ */ __name(async ({ request }) => {
  ...
  if (!feedUrl || !ALLOWED_FEEDS.has(feedUrl)) {
    return new Response("Forbidden", { status: 403, headers: CORS2 });
  }
  try {
    ...
    return new Response(xml, {
      status: 200,
      headers: { ...CORS2, "Content-Type": "application/xml; charset=utf-8" }
    });
  } catch {
    return new Response("Feed unavailable", { status: 502, headers: CORS2 });
  }
}, "onRequest");

// ../.wrangler/tmp/pages-inMW6d/functionsRoutes-0.5864960194793232.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-z6Vcih/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-z6Vcih/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.18297747296842193.mjs.map
